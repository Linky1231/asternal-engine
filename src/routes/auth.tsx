...
function AmbientDot({ index }: { index: number }) {
  const size = 2 + (index % 2);
  const x = `${(index * 13 + 7) % 100}%`;
  ...
  return (
    <div
      ...
      style={{
        ...
        animation: `float-particle ${5 + (index % 3)}s ease-in-out ${index * 0.8}s infinite`,
        background: index % 2 === 0
          ? "oklch(0.488 0.185 264 / 0.08)"
          : "oklch(0.72 0.11 260 / 0.06)",
        boxShadow: `0 0 ${size * 3}px oklch(0.488 0.185 264 / 0.04)`,
      }}
    />
  );
}
  ...
  return (
    <div ref={cardRef} onMouseMove={onMove}
      ...
      <div className="relative" style={{
        ...
        <div className="absolute inset-0 pointer-events-none rounded-2xl opacity-0 transition-opacity duration-700"
          style={{
            opacity: hover ? 0.5 : 0,
  ...
  return (
    <div className="min-h-screen w-screen flex flex-col bg-background overflow-y-auto relative">
      ...
      <div className="fixed inset-0 pointer-events-none select-none overflow-hidden">
        ...
        <div className="absolute -top-[30%] -left-[20%] w-[70%] h-[70%] opacity-40"
          style={{
        ...
        />
        <div className="absolute -bottom-[30%] -right-[20%] w-[70%] h-[70%] opacity-40"
          style={{
            background: "radial-gradient(circle at 70% 50%, oklch(0.72 0.11 260 / 0.05), transparent 60%)",
            animation: "drift-slow 25s ease-in-out infinite reverse",
          }}
        />

        {/* Ambient floating dots (very subtle) */}
        {Array.from({ length: 8 }).map((_, i) => <AmbientDot key={i} index={i} />)}
        ...
        <div className={`flex justify-center transition-all duration-1000 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
          ...
          <Link to="/" className="inline-flex items-center gap-2.5 group mb-10">
            <div className="relative">
              ...
              <div className="w-8 h-8 rounded-lg bg-primary grid place-items-center shadow-lg shadow-primary/25 relative">
                <Gamepad2 size={16} className="text-white" />
              </div>
            </div>
            <span className="text-base font-display font-semibold tracking-tight bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">
              Asternal
            </span>
          </Link>
        </div>
          ...
          <div className={`w-full max-w-sm shrink-0 transition-all duration-1000 delay-200 ${loaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
            ...
            <TiltCard>
              <div className="relative bg-white/70 backdrop-blur-2xl rounded-2xl border border-white/50 shadow-2xl shadow-primary/[0.08] p-7 overflow-hidden group/form-card">
                ...
                <div className="absolute inset-0 rounded-2xl p-[1px] pointer-events-none opacity-0 group-hover/form-card:opacity-100 transition-opacity duration-700">
                  <div className="absolute inset-0 rounded-2xl bg-gradient-to-r from-transparent via-primary/20 via-accent/15 to-transparent bg-[length:200%_100%] animate-shimmer-border" />
                </div>
                ...
                <div className="text-center mb-6">
                  <div className="w-11 h-11 rounded-2xl bg-primary/10 grid place-items-center mx-auto mb-3 relative">
                    <div className="absolute inset-0 rounded-2xl bg-primary/5 blur-md scale-150 animate-pulse" style={{ animationDuration: '3s' }} />
                    <Gamepad2 size={20} className="text-primary relative" />
                  </div>
