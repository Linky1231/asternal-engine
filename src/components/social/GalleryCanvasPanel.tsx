  ...
  return (
    <div className="w-full h-full flex flex-col rounded-2xl border border-border/40 panel overflow-hidden bg-muted/95 backdrop-blur-xl">
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0">
        ...
        {/* Tools sidebar - balanced size */}
        <div className="lg:w-[220px] shrink-0 border-t lg:border-t-0 lg:border-l border-border/30 overflow-y-auto bg-muted/20">
          <div className="p-2.5 space-y-2">
            ...
            <div className="grid grid-cols-3 gap-1.5 p-2 rounded-xl bg-muted/80 border border-border/40">
              {TOOLS.map(t => (
                <button key={t.id} title={t.title} onClick={() => setTool(t.id)}
                  className={`h-10 rounded-xl text-sm transition-all flex items-center justify-center ${
                    tool === t.id
            ...
            <div className="flex gap-2">
              <button onClick={undo} title="Deshacer" className="flex-1 h-10 rounded-xl border border-border/50 bg-muted/30 text-foreground/70 hover:text-foreground hover:bg-muted/60 transition flex items-center justify-center active:scale-95"><Undo2 size={16} /></button>
              <button onClick={redo} title="Rehacer" className="flex-1 h-10 rounded-xl border border-border/50 bg-muted/30 text-foreground/70 hover:text-foreground hover:bg-muted/60 transition flex items-center justify-center active:scale-95"><Redo2 size={16} /></button>
              <button onClick={clearAll} title="Limpiar" className="flex-1 h-10 rounded-xl border border-destructive/30 bg-destructive/10 text-destructive hover:bg-destructive/20 transition flex items-center justify-center active:scale-95"><Trash2 size={16} /></button>
            </div>
            ...
            <div className="p-3 rounded-xl bg-muted/50 border border-border/40 space-y-2">
              <div className="flex items-center gap-2">
                <div className="relative w-10 h-10 shrink-0">
                  ...
                  <div className="w-10 h-10 rounded-xl pointer-events-none ring-1 ring-white/20" style={{ background: color, boxShadow: "0 4px 12px -4px oklch(0 0 0 / 0.4)" }} />
                </div>
                <input type="text" value={color.toUpperCase()} onChange={(e) => { const v = e.target.value.trim(); if (/^#?[0-9a-fA-F]{6}$/.test(v)) setColor(v.startsWith("#") ? v : `#${v}`); }}
                  className="bg-input/50 border border-border/50 rounded-lg px-2.5 py-1.5 text-xs font-mono w-20 tracking-tight focus:outline-none focus:border-primary/40 uppercase" maxLength={7} aria-label="Hex color" />
              </div>
              <div className="grid grid-cols-8 gap-1.5">
                {PALETTE.map((c) => (
                  <button key={c} onClick={() => setColor(c)} className="aspect-square rounded-lg transition-all min-h-[22px]" style={{
                    background: c,
              ...
              <div className="flex items-center gap-2.5">
                <span className="text-[11px] font-medium text-muted-foreground/80 w-14 shrink-0">Grosor</span>
              ...
              <div className="flex gap-4">
                <label className="flex items-center justify-between cursor-pointer select-none gap-2 flex-1">
                  ...
                  <span role="switch" aria-checked={stabilize} onClick={() => setStabilize(!stabilize)} className="relative w-[36px] h-[18px] rounded-full transition-colors shrink-0" style={{ background: stabilize ? "oklch(0.7 0.17 145)" : "oklch(0.35 0.02 260)", boxShadow: "inset 0 1px 2px oklch(0 0 0 / 0.3)" }}>
                    <span className="absolute top-[2px] w-[14px] h-[14px] rounded-full bg-white transition-all" style={{ left: stabilize ? "20px" : "2px", boxShadow: "0 2px 4px oklch(0 0 0 / 0.3)" }} />
                  </span>
                </label>
                <label className="flex items-center justify-between cursor-pointer select-none gap-2 flex-1">
                  <span className="text-xs font-medium text-muted-foreground/80">Pres</span>
                  <span role="switch" aria-checked={pressureOn} onClick={() => setPressureOn(!pressureOn)} className="relative w-[36px] h-[18px] rounded-full transition-colors shrink-0" style={{ background: pressureOn ? "oklch(0.7 0.17 145)" : "oklch(0.35 0.02 260)", boxShadow: "inset 0 1px 2px oklch(0 0 0 / 0.3)" }}>
                    <span className="absolute top-[2px] w-[14px] h-[14px] rounded-full bg-white transition-all" style={{ left: pressureOn ? "20px" : "2px", boxShadow: "0 2px 4px oklch(0 0 0 / 0.3)" }} />
                  </span>
                </label>
              </div>
            </div>
            ...
            <div className="p-3 rounded-xl bg-muted/50 border border-border/40 space-y-2">
              <button onClick={() => setToolsExpanded(e => !e)} className="flex items-center justify-between w-full">
                <span className="text-xs font-semibold tracking-wider text-muted-foreground/90 inline-flex items-center gap-1.5">
                  <Layers size={14} /> CAPAS · {layers.length}
                </span>
                <button onClick={(e) => { e.stopPropagation(); addLayer(); }} className="w-7 h-7 rounded-lg flex items-center justify-center text-primary-foreground active:scale-95 transition" style={{ background: "linear-gradient(180deg, oklch(0.78 0.17 250), oklch(0.66 0.18 252))" }}>
              ...
              {toolsExpanded && (
                <div className="flex flex-col gap-1 max-h-[200px] overflow-auto">
                  {[...layers].slice().reverse().map((l) => {
                    ...
                    return (
                      <div key={l.id} className={`group rounded-xl p-2 flex items-center gap-2 transition-all cursor-pointer ${
                        isActive
                          ? "bg-primary/10 border border-primary/30"
                          : "bg-muted/40 border border-border/40 hover:bg-muted/60"
                      }`}
                        onClick={() => { setActiveLayerId(l.id); activeLayerIdRef.current = l.id; }}>
                        <div className="w-8 h-8 rounded-lg shrink-0 overflow-hidden" style={{
                          backgroundColor: "#ffffff",
                        ...
                        <input value={l.name} onChange={(e) => updateLayer(l.id, { name: e.target.value })} onClick={(e) => e.stopPropagation()}
                          className="flex-1 min-w-0 bg-transparent text-[11px] font-medium focus:outline-none focus:bg-muted/50 rounded px-1 py-0.5" />
