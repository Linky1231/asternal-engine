...
import {
  Database, Loader2, CheckCircle2, AlertTriangle, ShieldCheck, KeyRound, ExternalLink, Plug,
} from "lucide-react";
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md panel border-border/60 rounded-2xl">
        ...
        <div className="space-y-3 pt-1">
          ...
          {status === "local" && (
            <div className="rounded-xl border border-amber-200/60 bg-amber-50/60 dark:bg-amber-950/20 dark:border-amber-800/40 p-3.5 text-xs text-amber-800 dark:text-amber-300 space-y-2">
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <div className="text-amber-700/80 dark:text-amber-300/70 mt-0.5">
                    La app funciona con datos guardados en este navegador. Para sincronizar entre
                    dispositivos, añade estas claves en el apartado <b>Keys / API keys</b> de este proyecto:
                  </div>
                </div>
              </div>
              <div className="font-mono text-[11px] bg-white/70 dark:bg-black/20 rounded-lg px-3 py-2 space-y-0.5">
                <div><b>VITE_SUPABASE_URL</b> → https://xxxx.supabase.co</div>
                <div><b>VITE_SUPABASE_ANON_KEY</b> → sb_publishable_… / eyJhbGci…</div>
              </div>
            </div>
          )}
