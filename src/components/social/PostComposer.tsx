...
export function PostComposer({ onCreated }: { onCreated: () => void }) {
  ...
  const Chip = ({ active, onClick, title, children }: { active?: boolean; onClick: () => void; title: string; children: React.ReactNode }) => (
    <button onClick={onClick} title={title}
      ...
      <span aria-hidden className={`absolute inset-0 rounded-xl bg-gradient-to-r from-primary to-accent transition-opacity duration-300 ease-out ${active ? "opacity-100" : "opacity-0"}`} />
      ...
      <div className="p-3 space-y-3">
        ...
        {expanded && (
          <div className="text-[10px] font-display tracking-[0.2em] px-1 flex items-center gap-2">
            ...
            <span className="bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">AÑADIR A TU PUBLICACIÓN</span>
